...
def save_nifti(img_data, ps, filename=None, ref_path=None, debug_level=0) -> None:
    ...
    p = Path(os.path.join(out_path, f'{filename}'))
    nib.save(img, p.with_suffix('.nii.gz'))
    # save bvals
    np.savetxt(p.with_suffix('.bval'), BVALS.reshape(1, len(BVALS)), fmt='%d', delimiter=' ')
    # save unit bvecs
    bvecs = np.zeros((3, len(BVALS)))
    bvecs[0, :] = 1  # x direction
    np.savetxt(p.with_suffix('.bvec'), bvecs, fmt='%d', delimiter=' ')

    if debug_level >= 1:
        print(f"Saved NIfTI file to: {p.with_suffix('.nii.gz')}")
        print(f"Saved bval file to: {p.with_suffix('.bval')}")
        print(f"Saved bvec file to: {p.with_suffix('.bvec')}")
...
save_nifti(np.abs(sense_prelim_all), ps, filename=pref+"sense_prelim_all_abs", ref_path=ref_path, debug_level=debug_level)
b0 = np.abs(sense_prelim_all.squeeze())[..., 0] + 1e-8  # avoid div by zero
save_nifti(np.abs(sense_prelim_all) / b0[..., None], ps, filename=pref+"sense_prelim_all_abs_removeb0", ref_path=ref_path, debug_level=debug_level)

sense_prelim_all_rot180 = np.rot90(sense_prelim_all, k=2, axes=(0,1))
save_nifti(sense_prelim_all_rot180, ps, filename=pref+"sense_prelim_all_rot180", ref_path=ref_path, debug_level=debug_level)
save_nifti(np.abs(sense_prelim_all_rot180), ps, filename=pref+"sense_prelim_all_rot180_abs", ref_path=ref_path, debug_level=debug_level)
b0_rot180 = np.abs(sense_prelim_all_rot180.squeeze())[..., 0] + 1e-8  # avoid div by zero
save_nifti(np.abs(sense_prelim_all_rot180) / b0_rot180[..., None], ps, filename=pref+"sense_prelim_all_rot180_abs_removeb0", ref_path=ref_path, debug_level=debug_level)
# %%
s = mdm.mdm_s_from_nii(os.path.join(ps.nii_p, "6_STEs_2mmiso_PA_4basis_bdelta0_bak_sense_prelim_abs.nii.gz"), b_delta=0)
print(s)
# %%